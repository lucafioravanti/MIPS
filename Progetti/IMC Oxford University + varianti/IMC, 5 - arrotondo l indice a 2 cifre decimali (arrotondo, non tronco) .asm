...
.data	
	...
	cento:			.float	100			# costante per arrotondamento
	mezzo: 			.float	0.5			# costante per arrotondamento
...
exp_done:
	...
	# START arrotondo indice a 2 cifre decimali
	
	l.s $f20, cento			# carico la costante 100.0 in $f20
	l.s $f21, mezzo			# carico la costante 0.5 in $f21
	  
	mul.s $f6, $f20, $f6		# moltiplico IMC * 100
	add.s $f6, $f6, $f21		# aggiungo 0.5 
	cvt.w.s $f6, $f6 		# IMC: float -> int 
	cvt.s.w $f6, $f6 		# IMC: int -> float
	div.s $f6, $f6, $f20		# divido per 100
		
	# END arrotondo indice a 2 cifre decimali
