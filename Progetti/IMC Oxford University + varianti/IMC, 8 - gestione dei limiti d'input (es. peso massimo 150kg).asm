...
.data	
	...
	s_err_in_p:		.asciiz "Errore: massimo peso inseribile: 150 [kg]\n"
	s_err_in_h:		.asciiz "Errore: massima altezza inseribile: 2.10 [m]\n"
	max_h:			.float	2.1			# massima altezza inseribile 2 metri e 10
...
main:
	...
	### controllo che l'input non superi il valore massimo
	
	li $t0, 150			# supponendo 150 come limite inseribile per il peso in [kg], carico la costante in t0
	bgt $s0, $t0, ERRP	# goto ERR if s0 > t0
	b continue
...
continue:
	...
	l.s $f1, max_h		# 'max_h' limite inseribile per l'altezza in [m]
	c.le.s $f10, $f1 	# confronto
	bc1f ERRH 			# salta se f1 > f10 (cioe' f1 < f10 e' falso)
